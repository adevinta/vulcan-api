...
annotations already present in the asset.`,
	...
	tmp29 := new(DiscoverAssetsCommand)
	sub = &cobra.Command{
...

{
   ...
   "group_name": "discoveryserviceX-discovered-assets"
	...
	tmp30 := new(EmailScanReportCommand)
	sub = &cobra.Command{
...
func (cmd *CurrentExposureGlobalStatsCommand) Run(c *client.Client, args []string) error {
	...
	resp, err := c.CurrentExposureGlobalStats(ctx, path, tmp103, tmp104, stringFlagVal("tags", cmd.Tags))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *ExposureGlobalStatsCommand) Run(c *client.Client, args []string) error {
	...
	resp, err := c.ExposureGlobalStats(ctx, path, stringFlagVal("atDate", cmd.AtDate), tmp105, tmp106, stringFlagVal("tags", cmd.Tags))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
